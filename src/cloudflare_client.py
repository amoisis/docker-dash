...
def _is_retryable_api_error(error):
    """
    Determine if a Cloudflare APIError is retryable.
    Retries 5xx, 429 (rate limit), and network-level errors.
    Does not retry 4xx client errors (except 429).
    """
    status_code = getattr(error, 'status_code', None)
    if status_code is not None:
        return status_code >= 500 or status_code == 429
    # If status_code is unavailable, retry based on message heuristics
    message = str(error).lower()
    return any(term in message for term in ['rate limit', 'timeout', 'connection', 'temporary', 'unavailable'])
                ...
                try:
                    ...
                    if attempt < max_retries - 1 and _is_retryable_api_error(e):
                        ...
                        if not _is_retryable_api_error(e):
                            logging.error(
                                f"Cloudflare API client error in {func.__name__}: {e}. Not retrying."
                            )
                        else:
                            logging.error(
...
def validate_service_url(service: str) -> None:
    ...
    url_pattern = re.compile(
        ...
        r'([a-zA-Z0-9]([a-zA-Z0-9-_]{0,61}[a-zA-Z0-9])?\.)*[a-zA-Z0-9]([a-zA-Z0-9-_]{0,61}[a-zA-Z0-9])?'  # hostname (allows underscores for Docker service names)
...
def populate_tunnel_cache(cf_client, account_id, manager=None):
    ...
    try:
        ...
        with manager._cache_lock:
            manager.tunnel_cache.clear()
            manager.tunnel_cache.update(new_cache)
...
def populate_access_caches(cf_client, account_id, manager=None):
    ...
    try:
        ...
        new_apps = {app.domain: app for app in apps}
        new_policies = {policy.name: policy for policy in policies}
        new_idps = {idp.name: idp for idp in idps}

        with manager._cache_lock:
            manager.access_apps_cache.clear()
            manager.access_apps_cache.update(new_apps)
            manager.access_policies_cache.clear()
            manager.access_policies_cache.update(new_policies)
            manager.idps_cache.clear()
            manager.idps_cache.update(new_idps)

        logging.info(f"Cached {len(new_apps)} Access Apps, "
                     f"{len(new_policies)} Policies, and {len(new_idps)} IDPs.")
    except cloudflare.APIError as e:
...
def populate_zones_cache(cf_client, manager=None):
    ...
    try:
        ...
        new_zones = {zone.name: zone for zone in zones}

        with manager._cache_lock:
            manager.zones_cache.clear()
            manager.zones_cache.update(new_zones)

        logging.info(f"Cached {len(new_zones)} DNS zones.")
        logging.debug(f"Zone names in cache: {list(new_zones.keys())}")
    except cloudflare.APIError as e:
...
def get_cached_tunnels(manager=None):
    ...
    if manager is None:
        manager = _manager
    with manager._cache_lock:
        return dict(manager.tunnel_cache)
...
def get_consolidated_cache(manager=None):
    ...
    with manager._cache_lock:
        return {
            "tunnels": dict(manager.tunnel_cache),
            "access_applications": dict(manager.access_apps_cache),
            "access_policies": dict(manager.access_policies_cache),
            "identity_providers": dict(manager.idps_cache),
            "zones": dict(manager.zones_cache),
        }


def get_cache_health(manager=None):
    """Returns cache health metadata for the health endpoint.
    ...
    with manager._cache_lock:
        return {
            "status": "ok" if manager.last_cache_refresh else "unknown",
            "last_refresh": manager.last_cache_refresh.isoformat() if manager.last_cache_refresh else None,
            "tunnel_count": len(manager.tunnel_cache),
            "access_app_count": len(manager.access_apps_cache),
            "policy_count": len(manager.access_policies_cache),
            "idp_count": len(manager.idps_cache),
            "zone_count": len(manager.zones_cache),
        }
...
def periodic_cache_refresh(manager=None):
    ...
    while not manager._stop_refresh_event.wait(manager.cache_refresh_interval):
...
def stop_cache_refresh_thread(manager=None):
    ...
    if manager.cache_refresh_thread and manager.cache_refresh_thread.is_alive():
        ...
        manager._stop_refresh_event.set()
        manager.cache_refresh_thread.join(timeout=5)
        manager._stop_refresh_event.clear()
